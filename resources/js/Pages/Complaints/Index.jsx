...
export default function ComplaintsIndex({
    ...
    const submit = (e) => {
        ...
        if (editingId) {
            put(route('complaints.update', { resource: activeResource, id: editingId }), {
                preserveScroll: true,
        ...
        post(route('complaints.store', { resource: activeResource }), {
            preserveScroll: true,
            onSuccess: () => {
                reset();
                setData(buildDefaultState(fields));
    ...
    const handleDelete = (id) => {
        ...
        router.delete(route('complaints.destroy', { resource: activeResource, id }), {
            preserveScroll: true,
        });
    };

    return (
        <DashboardLayout title="Complaint Center">
            <div className="space-y-6">
                <div className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage complaints, complaint replies, and complaint attachments from one place.</p>
                ...
                {missingTable && (
                    <div className="rounded-xl border border-amber-300 bg-amber-50 text-amber-900 px-4 py-3 text-sm">
                        Missing database table: <span className="font-semibold">{missingTable}</span>. Please run migrations before using this resource.
                    </div>
                )}
