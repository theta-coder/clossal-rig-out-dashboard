...
import { HiOutlinePencil, HiOutlineStar, HiOutlineViewGrid } from 'react-icons/hi';
            ...
            <div className="max-w-4xl">
                <div className="mb-6 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <Link href={`/products/${product.id}/variants`} className="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-xl text-sm font-medium transition-all">
                            <HiOutlineViewGrid className="w-4 h-4" /> Manage Variants
                        </Link>
                    ...
                    <div className="lg:col-span-1 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4">
                        {product.images?.length > 0 ? (
                            <div className="space-y-3">
                                <img src={`/${product.images[0].image_path}`} className="w-full rounded-xl object-cover aspect-square" alt={product.name} />
                                <div className="grid grid-cols-4 gap-2">
                                    {product.images.slice(1).map((img) => <img key={img.id} src={`/${img.image_path}`} className="w-full rounded-lg object-cover aspect-square" alt={product.name} />)}
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {product.sizes?.length > 0 && (
                            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
                                ...
                                <div className="flex flex-wrap gap-2">{product.sizes.map((s) => <div key={s.id} className="px-4 py-2 rounded-xl bg-gray-50 dark:bg-gray-800 text-sm"><span className="font-medium text-gray-900 dark:text-white">{s.name}</span><span className="text-gray-400 ml-2">({s.pivot?.stock ?? 0} in stock)</span></div>)}</div>
                            </div>
                        )}
                        {/* Colors */}
                        {product.colors?.length > 0 && (
                            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
                                ...
                                <div className="flex flex-wrap gap-3">{product.colors.map((c) => <div key={c.id} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-50 dark:bg-gray-800"><div className="w-5 h-5 rounded-full border border-gray-200 dark:border-gray-600" style={{ backgroundColor: c.code || 'transparent' }}></div><span className="text-sm text-gray-700 dark:text-gray-300">{c.name}</span></div>)}</div>
                            </div>
                        )}
                        {/* Tags */}
                            ...
                            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {product.tags.map((tag) => (
                                        <span key={tag.id} className="px-3 py-1.5 rounded-lg text-xs border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
                                            {tag.name}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {product.collections.map((collection) => (
                                        <span key={collection.id} className="px-3 py-1.5 rounded-lg text-xs border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
                                            {collection.name}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
                                ...
                                <ul className="space-y-2">
                                    {product.details.map((detail) => (
                                        <li key={detail.id} className="text-sm text-gray-600 dark:text-gray-400">{detail.detail}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
