...
export default function CollectionsEdit({ collection, products }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        image: null,
        starts_at: collection.starts_at ? collection.starts_at.split(' ')[0] : '',
        ends_at: collection.ends_at ? collection.ends_at.split(' ')[0] : '',
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Tag Products</h2>
                        <div className="max-h-[500px] overflow-y-auto pr-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                            {products.map(product => (
                                ...
                                >
                                    ...
                                    <span className={`text-sm font-medium ${data.product_ids.includes(product.id) ? 'text-primary-700 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'} truncate`}>
                                        {product.name}
                                    </span>
                                </div>
                            ))}
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-4">
                        ...
                        <div className="relative group">
                            <div className="aspect-video rounded-xl bg-gray-50 dark:bg-gray-700/50 border-2 border-dashed border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center overflow-hidden">
                                {data.image ? (
                                    ...
                                    <img src={`/${collection.image}`} className="w-full h-full object-cover" alt="Current" />
                                ) : (
                        ...
                        <p className="text-[10px] text-gray-500 text-center uppercase tracking-widest">Click to upload new banner</p>
                    </div>
                    ...
                    >
                        <HiOutlineSave className="w-5 h-5" /> {processing ? 'Saving...' : 'Update Collection'}
                    </button>
                </div>
            </form>
        </DashboardLayout>
