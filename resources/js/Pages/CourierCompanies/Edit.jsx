...
export default function CourierCompaniesEdit({ company }) {
    const { data, setData, put, processing, errors } = useForm({
        name: company?.name ?? '',
        slug: company?.slug ?? '',
        logo: company?.logo ?? '',
        tracking_url_format: company?.tracking_url_format ?? '',
        api_key: company?.api_key ?? '',
        api_secret: company?.api_secret ?? '',
        api_url: company?.api_url ?? '',
        contact_number: company?.contact_number ?? '',
        contact_email: company?.contact_email ?? '',
        default_rate: company?.default_rate ?? '0',
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(`/courier-companies/${company.id}`);
    };

    return (
        <DashboardLayout title="Edit Courier Company">
