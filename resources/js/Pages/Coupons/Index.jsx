        ...
        <DashboardLayout title="Coupons">
            ...
            <DataTable url="/coupons" columns={columns} actions={(row) => (
                <>
                    ...
                    <button onClick={() => { if (confirm('Delete this coupon?')) router.delete(`/coupons/${row.id}`); }} className="p-2 rounded-lg text-gray-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10 dark:hover:text-red-400 transition-colors"><HiOutlineTrash className="w-4 h-4" /></button>
                </>
            )} />
        </DashboardLayout>
    );
